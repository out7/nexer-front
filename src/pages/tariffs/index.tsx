...
export default function TariffsPage() {
	...
	useEffect(() => {
		...
		;(async () => {
			try {
				...
				if (!isMounted) return
				setTariffs(data)
				const defaultTariff = data.find(t => t.months === 3) ?? data[0] ?? null
				setSelectedTariffId(
					defaultTariff
	...
	const handlePay = useCallback(async () => {
		...
		try {
			...
			const response = await api.get<{ url: string }>(`/invoice/tariff/${code}`)
		...
		<div className={styles.container} style={{ paddingTop }}>
			...
			<section className={styles.section}>
				...
				>
					{sortedTariffs.map(t => {
						const isSelected =
							selectedTariffId === t.id || String(t.months) === selectedTariffId
						return (
							<div
